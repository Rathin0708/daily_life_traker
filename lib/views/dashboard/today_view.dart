...
class TodayView extends StatelessWidget {
      ...
      flexibleSpace: FlexibleSpaceBar(
        ...
        background: Stack(
          ...
          children: [
            ...
            Center(
              child: ModernProgressRing(
                progress: score,
                size: 160,
                color: score >= 90 ? AppColors.success : AppColors.primary,
              ),
            ),
          ],
        ),
  ...
  Widget _buildDynamicHeader(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          user?.displayName != null ? 'Hello, ${user!.displayName}' : 'Forge Your Future',
          ...
          children: [
            ...
            Consumer<RoutineViewModel>(
              builder: (context, vm, _) {
                final stats = vm.getPriorityCompletionStats();
                final highComp = (stats['high_completion'] ?? 0) * 100;
                return Text(
                  '${highComp.toInt()}% HIGH PRIORITY',
                  style: TextStyle(
  ...
  Widget _buildBeautifulTaskItem(BuildContext context, LoggedTask task, int index, RoutineViewModel vm) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isCompleted ? AppColors.success.withValues(alpha: 0.3) : Colors.white10,
        ),
      ),
      child: ListTile(
          ...
          ElevatedButton.icon(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(builder: (_) => const NightReviewView()),
              );
            },
            icon: const Icon(Icons.nightlight_round),
            label: const Text('Complete Daily Review'),
          ),
        ],
      ),
    );
